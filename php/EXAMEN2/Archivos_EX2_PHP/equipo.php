...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['accion'])) {
    ...
    if ($accion == "reclutar") {
        ...
        $personaje = HEROES[$idHeore];
        if(in_array(HEROES['id'][$idHeore],$personaje)){
            $_SESSION['flash_message'] = "El heroe ya esta reclutado ";
        }
        
        if (!in_array($idHeore, $_SESSION['equipo'], true)) {
            $_SESSION['equipo'][] = $personaje;
    ...
    if ($accion == "eliminar") {
        ...
        $personaje = HEROES[$idHeore];
        if(!in_array(HEROES['id'][$idHeore],$personaje)){
            $_SESSION['flash_message'] = "El heroe no se encuentra ene l equipo ";
        }
        if (!in_array($idHeore, $_SESSION['equipo'], true)) {
            unset($_SESSION['equipo'][$idHeore]);
            $_SESSION['costeTotal'] -= HEROES[$idHeore]['coste'];
            $_SESSION['presupuesto'] += HEROES[$idHeore]['coste'];
            $_SESSION['flash_message'] = "El heroe se ha eliminado correctamente ";
        }
        header("Location: equipo.php");
        exit();
    }
